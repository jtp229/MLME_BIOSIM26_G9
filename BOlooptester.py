...
#Opens up a Basic Client object that is used to access the api
client = basic_client.BioreactorClient(basic_client.BASE_URL)
client.login(basic_client.USER, basic_client.PASSWORD)


#States the minimums and maximums as stated by the project specifications
RECIPE_MIN = np.array([20.0,  3.0, 0.0, 0.0, 0.0])
RECIPE_MAX = np.array([60.0,  9.5, 2.0, 2.0, 2.0])


#Small dictionay that maps numbers to the names of the tests
SCALE_MAPPING = {0: "micro", 1: "bench", 2: "pilot"}

#Scale costs for each of the tyeps of experiments
SCALE_COSTS = {0: 10.0, 1: 500.0, 2: 2000.0}

#Estimated noises coming from our New Noise Probe Scripts, basically the spread of each experiment. Weird results from Bench being so high
SCALE_NOISE = {
    0: 0.000365,   # micro 
    1: 0.003797,   # bench 
    2: 0.000024,   # pilot 
...
#Correlation matrix for the correlations between each of the different experiments
B = np.array([
    [1.00, 0.20, 0.15],
    [0.20, 1.00, 0.90],
    [0.15, 0.90, 1.00],
...
#Kernels are a shortcut for gaussian processes that allows you to not define non-linear features yet still get them. It is called multifidelity
#since it is taking in oth the data from the lower fidelity tests like micro and the higher fidelity tests like pilot
class MultiFidelityKernel(RBF):

    def __call__(self, X, Y=None, eval_gradient=False):
        #Takes the first 5 values in the X and Y recipe matrices. The 5 input values are contained in these first 5 elements and are taken to be
        #Put into the orginal RBF parnet function. Eval gradient is used to know if it should tune hyper paremetrs or not
        #The B scale is multiplied by the given RBF matrix to give the correlations 
        ...
        #For evaluting Gaussian Processes with kernels this math is done to get the kernel. Math is similair to what is shown in slides
        #Varies depending on if tuning should happen or not with the RBF gradient
...
#Takes in a distribution an returns the the expected improvment for an expected guess. Uses predicetion mean and standard deviation estmation to get the expected improvement
def expected_improvement(mean, std, best_f, xi = 0.05):
    if std <= 1e-6:
        return 0.0
...
#converts the matrix of inputs to a properly normalized input(Sets each variable on scale of 0-1)
def transform_X(X):
    X_scaled = X.copy()
    X_scaled[:, :5] = (X[:, :5] - RECIPE_MIN) / (RECIPE_MAX - RECIPE_MIN)
    return X_scaled

#Go through process to select the next experiment
def select_next_experiment(gp_model, X_train, Y_train ,n_candidates=1000):
    
    #Pulls out and makes a true or false vector based on which of the previosu test runs were done with the Pilot scale
    ...
    #Take 1000(or whatever n_candidates is) randomly generated canidates and evaluate them using the guassian process that we have passed through
    ...
    #Go through and check each point for their expected improvement on the overall highest Y value in the pilot scale
 
    acqs = np.array([expected_improvement(m, s, best_pilot_f, xi=0.01) for m, s in zip(means, stds)])

    
    #Maximizer that returns negative value so minimizer from scikit can be used in a moment
    ...
    # Take the top 5 of the canidates that were generated and sort them
    top_k = min(5, n_candidates)
    top_indices = np.argsort(acqs)[-top_k:]
    
    best_acq_value = -np.inf
    best_recipe_scaled = None

    #Put the top canidates through a gradient based optimizer that try to futher optimize the top values predicted
    for idx in top_indices:
        start_recipe = random_recipes_scaled[idx]
        res = minimize(neg_acq, x0=start_recipe, method='L-BFGS-B', bounds=[(0, 1)] * 5
        )
        
        # Since neg_acq returns negative acquisition, minimizing it maximizes actual acquisition
        current_acq_value = -res.fun
        if current_acq_value > best_acq_value:
            best_acq_value = current_acq_value
            best_recipe_scaled = res.x

    #Scale selection based on uncertainty at the optimized recipe
    ...
    #Sets the best scale bast on the standard deviation at each scale
    pilot_std = float(stds_at_scales[2])
    if pilot_std > 0.99: best_scale = 0
    elif pilot_std > 0.44: best_scale = 1
    else: best_scale = 2

    #Undoes normalization of the recipe values
    best_recipe = best_recipe_scaled * (RECIPE_MAX - RECIPE_MIN) + RECIPE_MIN
    return best_recipe, best_scale



#Runs an experiment using inputted scale and recipe
...
def run_campaign(campaign_id, seed):
    ...
    #Use QMC to create a latin hypercube of the points and uses the seed 
    sampler = qmc.LatinHypercube(d=5, seed=seed)
    lhs_sample = sampler.random(n=N_INITIAL_SAMPLES)
    initial_recipes = qmc.scale(lhs_sample, RECIPE_MIN, RECIPE_MAX)
    
    #Run the latin hyper cube and output the values 
    for recipe in initial_recipes:   
        ...
        history.append({"T": float(recipe[0]), "pH": float(recipe[1]), "F1": float(recipe[2]), "F2": float(recipe[3]), "F3": float(recipe[4]), "scale": INITIAL_SCALE, "observed_Y": y, "cost_eur": cost, "cumulative_cost": total_spent, "source": "lhs_seed",
        })

    #Takes in all of the previously done tests
    X_train = np.array(X_list)
    Y_train = np.array(Y_list)

    #Initilize kernel and gausian process
    mf_kernel  = MultiFidelityKernel(length_scale=0.2, length_scale_bounds=(0.15, 1.5))
    gp = GaussianProcessRegressor(kernel=mf_kernel, alpha=np.array([SCALE_NOISE[int(s)] for s in X_train[:, 5]]), n_restarts_optimizer=5, normalize_y=True,)



    #Goes through budget testing canidates using select next, once it hits 2000 euros left it will run one lsat test using the pilot scale
    #Also puts values into arrays for later logging

    print("\nRunning BO Loop")
    while total_spent < BUDGET_LIMIT:
        remaining = BUDGET_LIMIT - total_spent
        print(f"\nBudget: {total_spent:.0f} / {BUDGET_LIMIT:.0f} EUR  (remaining: {remaining:.0f}€)")

        gp.alpha = np.array([SCALE_NOISE[int(s)] for s in X_train[:, 5]])

        if remaining <= PILOT_RESERVE:
            print("\nReached 2000 euros left")
            ...
            X_cand_pilot = np.column_stack([ random_recipes_scaled, np.full(exploitation_candidates, 2, dtype=float)])
        ...
        history.append({"T": float(next_recipe[0]), "pH": float(next_recipe[1]), "F1": float(next_recipe[2]), "F2": float(next_recipe[3]), "F3": float(next_recipe[4]),"scale": next_scale, "observed_Y": y, "cost_eur": cost, "cumulative_cost": total_spent, "source": "bo_loop" if remaining > PILOT_RESERVE else "final_pilot_exploitation",})
